...
#if defined (HTC_SUPPORT) && defined (__MINGW32__)
      /* Win32 treats file names case-insensitively (well, mostly -- but
         that's another story), so "FOO" and "foO" denote the same file name;
	 furthermore, both slashes and backslashes may be used as a path
	 separator.  The Cygwin environment takes great pains to transform
	 file names into a generic format, but does so at the expense of
	 adding a fairly complete POSIX-layer on top of the Win32 API,
	 which causes perfomance and interoperability issues between
	 native and Cygwin applications (leave alone compatibility issues
	 due to different versions of the Cygwin DLL).  That's reason
	 enough for some people to prefer the MinGW version of the GNU
	 tools, which make direct use of the Win32 API.  This, of course,
	 bears a different set of trade-offs, including the ambiguity of
	 file names, which makes it unsafe to simply use "strcmp" in order
	 to find out if two given file names point to the same file.

	 Strictly speaking, this problem can even occur in a Unix environment.
	 For instance, if there's a hard or symbolic link between the files
	 "foo.o" and "bar.o", and you pass "foo.o" on the linker's command
	 line, but specify "bar.o(.text)" in the linker script, the above
	 call to "strcmp" will report inequality between "foo.o" and "bar.o",
	 leading to adding "bar.o" to the list of input files and again
	 reading "foo.o"'s symbols, which usually results in error messages
	 complaining about multiply defined symbols.  The same happens if you
	 specify "foo.o" and "./foo.o" or "././foo.o" at different places.
	 Come to think of it, NLS might also add to the complexity...

	 A possible solution to address this problem would be to transform
	 all file names into some native format, so that "strcmp" could
	 be safely used to find out if two given file names are identical.
	 Both POSIX and GLIBC offer such a functionality (implemented by
	 "canonicalize_file_name ()"), but this works only for actually
	 existing file names, and is currently not supported by MinGW32.
	 Another option would be to "stat ()" both file names and see if
	 their i-node and device numbers match.  Hopefully, there will be
	 some discussion amongst the binutils developers that will eventually
	 lead to a proper solution.  In the meantime, the code below helps
	 addressing the MinGW-specific issues mentioned above.  */
      if (((search->filename == (char *) NULL) || (name == (char *) NULL))
	  || (strlen (search->filename) != strlen (name)))
        continue;
      else if (strcasecmp (search->filename, name) == 0)
	break;
      else
        {
	  const char *s, *d;
	  ...
	  for (s = search->filename, d = name; *s != '\0'; ++s, ++d)
	    {
...
#endif /* defined (HTC_SUPPORT) && defined (__MINGW32__)  */
    }
