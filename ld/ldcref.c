     ...
     struct cref_hash_entry *h;
{
  ...
  if (len != 0)
    {
      /* Symbol is either undefined or absolutely defined (via --defsym
         or via an assignment statement in the linker script).  */
      if (hl && (hl->type == bfd_link_hash_defined))
        {
	  if (bfd_is_abs_section (hl->u.def.section))
	    fprintf (fp, "%*s*ABS*\n", FILECOL - len, " ");
	  else if (bfd_is_com_section (hl->u.def.section)) 
	    fprintf (fp, "%*s*COM*\n", FILECOL - len, " ");
	  else
	    fprintf (fp, "%*s%s\n", FILECOL - len, " ",
	    	     bfd_archive_filename (hl->u.def.section->owner));
	}
      else
	fprintf (fp, "%*s*UND*\n", FILECOL - len, " ");
      len = 0;
